...
mod tests {
    ...
    fn test_rest_api_error_payload() {
        ...
        assert_eq!(payload.context().get("key"), Some(&json!("value")));
    }
    ...
    async fn test_error_reqwest() {
        let error = reqwest::get("not a url")
            .await
            .expect_err("This should not be a valid URL");
    ...
    fn test_invalid_header_value() {
        let error =
            HeaderValue::from_str("\u{0}").expect_err("This should not be a valid header value");
        let rest_api_error: RestApiError = error.into();
        assert_eq!(
    ...
    fn test_from_serde_json_error() {
        let error =
            serde_json::from_str::<Value>("{").expect_err("This should not be a JSON value");
        let rest_api_error: RestApiError = error.into();
        assert_eq!(
            rest_api_error.to_string(),
